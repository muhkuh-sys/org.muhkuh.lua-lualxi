...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.1-lualxi-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.1-lualxi-*.xml'))
if len(astrMatch)!=1:
    raise Exception('No match found for "lua5.1-lualxi-*.xml".')
...
subprocess.check_call('%s test' % strMake, shell=True, cwd=strCwd, env=astrEnv)
...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.4-lualxi-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.4-lualxi-*.xml'))
if len(astrMatch)!=1:
    raise Exception('No match found for "lua5.4-lualxi-*.xml".')
